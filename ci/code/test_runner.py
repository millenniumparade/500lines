...
class TestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        if command == "ping":
            ...
            self.request.sendall(str.encode("pong"))
        elif command == "runtest":
            logging.info("got runtest command: am I busy? %s" % self.server.busy)
            if self.server.busy:
                self.request.sendall(str.encode("BUSY"))
            else:
            ...
            if (time.time() - server.last_communication) > 10:
                try:
                    response = helpers.communicate(
                                       ...
                                       "status")
                    if response != "OK":
                        logging.info("Dispatcher is no longer functional")
                        server.shutdown()
                        return
                except socket.error as e:
                    logging.info("Can't communicate with dispatcher: %s" % e)
                    server.shutdown()
                    return
