...
def dispatch_tests(server, commit_id):
    ...
    while True:
        logging.info("trying to dispatch to runners")
        for runner in server.runners:
            response = helpers.communicate(runner["host"],
                                           int(runner["port"]),
                                           "runtest:%s" % commit_id)
            if response == "OK":
                logging.info( "adding id %s" % commit_id)
    ...
    def handle(self):
        ...
        if command == "status":
            ...
            if not self.server.runners:
                self.request.sendall(str.encode("No runners are registered"))
            else:
                # The coordinator can trust us to dispatch the test
                self.request.sendall(str.encode("OK"))
                dispatch_tests(self.server, commit_id)
        elif command == "results":
            logging.info("got test results")
            ...
            if length_msg > remaining_buffer:
                self.data += self.request.recv(length_msg - remaining_buffer).strip().decode()
...
def serve():
    ...
    logging.info('serving on %s:%s' % (args.host, int(args.port)))
            ...
            for runner in server.runners:
                ...
                try:
                    response = helpers.communicate(runner["host"],
                                                   ...
                                                   "ping")
                    if response != "pong":
                        logging.info("removing runner %s" % runner)
    ...
    def redistribute(server):
        while not server.dead:
            for commit in server.pending_commits:
                logging.info("running redistribute")
                logging.info(server.pending_commits)
